...
class LitBuildScoring(lt.LightningModule):
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(self.parameters(), lr=1e-2, weight_decay=1e-1)
    ...
    def training_step(self, train_batch, batch_idx):
        (meta_idx, decoy_idx, embedding_idx, system, dtag, event_num), z, m, rmsd, y = train_batch
        y = y.view(y.size(0), -1)
            ...
            self.train_annotations.append(
                {
                    ...
                    "y": 0.1 * float(np.argmax(y[j].cpu())),

                    # "y_hat": float(score[j]).to(torch.device("cpu")).detach().numpy()),
                    "y_hat": 0.1 * float(np.argmax(score[j].cpu())),
