...
class LitWaterScoring(lt.LightningModule):
    ...
    def training_step(self, train_batch, batch_idx):
        meta, z, y, centroid = train_batch
        idx, data_id,landmark_id, dtag, chain, seqid = meta
        y = y.view(y.size(0), -1)
        y = y.view(y.size(0), -1)
            ...
            self.train_annotations.append(
                {
                    ...
                    "data_idx": int(data_id[j].to(torch.device("cpu")).detach().numpy()),
                    'landmark_idx': int(landmark_id[j].to(torch.device("cpu")).detach().numpy()),
                    'dtag': str(dtag[j]),
                    'chain': str(chain[j]),
                    'seqid': str(seqid[j]),
