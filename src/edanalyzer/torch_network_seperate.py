...
class Classifier(nn.Module):
    def __init__(self, num_classes, block):
        self.fc1 = nn.Linear(2 * 512 * block.expansion, 512 * block.expansion)
        self.drop1 = nn.Dropout()
        self.fc2 = nn.Linear(512 * block.expansion, 256 * block.expansion)
        self.drop2 = nn.Dropout()
        self.fc3 = nn.Linear(256 * block.expansion, num_classes)
        self.act = nn.Softmax()

    def forward(self, density_encoding, ligand_encoding):
        x = torch.cat((density_encoding, ligand_encoding))
        x = self.fc1(x)
        x = self.drop1(x)
        x = self.fc2(x)
        x = self.drop2(x)
...
class EncoderClassifier(nn.Module):
    def __init__(self):
        self.density_encoder = ResNetEncoder(
            BasicBlock, [2, 2, 2, 2], num_input=2
        )
        self.ligand_encoder = ResNetEncoder(
            BasicBlock, [2, 2, 2, 2], num_input=1
        )
        self.classifier = Classifier(2, BasicBlock)

    def forward(self, x):
        density_layers = x[:,0:2,:,:,:]
        ligand_layers = x[:,2,:,:,:]
        density_encoding = self.density_encoder(density_layers)
        ligand_encoding = self.ligand_ecoding(ligand_layers)
        classification = self.classifier(density_encoding, ligand_encoding)
