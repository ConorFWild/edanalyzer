...
def _get_ligand_pdb_path(pandda_dir, dtag):
    ...
    pdb_paths = [x for x in paths if (x.exists()) and (x.stem not in constants.LIGAND_IGNORE_REGEXES)]
    ...
    for visits in Path('/dls/labxchem/data').glob('*'):
        for visit in visits.glob('*'):
            ...
            for dataset_dir in data_dir.glob('*'):
                ...
                model_path = (dataset_dir / 'refine.pdb').resolve()
                ...
                for lig_id, data in matches_by_pandda[best_pandda_match].items():
                    chain, resid = lig_id
                    if data[0] in [_x[0] for _x in ligands_to_event_maps.values()]:
                        print(f"MULTIPLE LIGANDS MAPPED TO SAME EVENT MAP! SKIPPING ADDITIONAL LIGANDS!")
                    ...
                    for ligand_key, ligand_graph_automorphisms in ligand_graphs.items():
                        for known_hit_key, known_hit in dtag_known_hits.items():
                            ...
                            rmsds[(ligand_key, known_hit_key)] = {
                                ...
                                "Known_Hit_Key": selected_known_hit_key,
                                # "Autobuild Key": autobuild_key[1],
                                "Ligand_Key": ligand_key,
                                "RMSD": rmsds[selected_known_hit_key],
