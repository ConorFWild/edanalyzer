...
def main(config_path, batch_size=12, num_workers=None):
    ...
    checkpoint_callback = ModelCheckpoint(dirpath='output/event_scoring_frag_adam_fast')
    logger = CSVLogger("output/event_scoring_frag_adam_fast/logs")
    trainer = lt.Trainer(accelerator='gpu', logger=logger,
                         callbacks=[
                             checkpoint_callback,
                             StochasticWeightAveraging(swa_lrs=1e-2, swa_epoch_start=0.4)
                         ],
